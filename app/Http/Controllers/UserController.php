...
class UserController extends Controller {
    ...
    public function getEdit(Request $request, $id = 0) {
        ...
        return view('user.formulaire')->with('user', $user);
    }
    ...
    public function postStore(Request $request) {
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                return redirect('users/edit/' . $request->input('id'))->withInput()->withErrors($validator->messages());
            }
            ...
            if (Help::checkObject(new User(), 'email', $request->input('phone'), $request->input('id', 0)))
                $error = "Numéro de téléphone déja enregistré";

            if (strlen(trim($error)) > 0)
                return view('user.formulaire', ['user' => $user, 'error' => $error]);
            ...
            return view('user.formulaire', ['user' => $user, 'success' => 1]);
        }
    }
